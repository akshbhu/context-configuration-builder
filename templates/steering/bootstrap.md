...
2. The ACTIVE context = only the projects marked ✅ in `context-registry.md`. Ignore non-enabled projects even if their skill files exist.
...
- Edit `context-registry.md` (flip ✅/⬜). That single edit is the source of truth for what's active.
