...
- **Metrics:** success rate (all tests pass, `exec_{R∘X̂}(T)=pass`); **# steps** (env interactions); **cost (USD)**; reasoning tokens.
...
- Success = real test-execution pass/fail via SWE-bench Docker grading (swebench_run.py `grade()`). ✓
- Per-(task,condition) logging of success + tokens + steps to JSONL. ✓
- temp=0, single sample. ✓
- Uses SWE-bench (add Lite split) + can add CTXbench. ✓
